    ...
    public function loginaction()
    {
        if(!empty($_POST['submit'])) {
            ...
            if ($validation->isValide($error)){
                ...
                $auth = $AuthentificationModel->isValidLoginInfo($username, $password);
                print_r($auth);
                //die();
                $model = new UserModel;
                $user = $model->findAll();
                ...
                if($auth == 0) {
                    ...
                    $error['username'] = 'Pseudo ou mot de passe incorrect';
                    ...
                    print_r($user);
                        ...
                        $authz = new AuthorizationModel;
                        $admin = $authz->isGranted('admin');
                        //$user  = $authz->isGranted('user');
                        //Si connexion OK redirection
                        print_r($admin);
                        if($authz->isGranted('admin')){
                            $AuthentificationModel->logUserIn($user);
                            $this->redirectToRoute('dashboard');
                        }   elseif($authz->isGranted('user'))
                            {
                                $AuthentificationModel->logUserIn($user);
        ...
        $departements       = $all->findAll();
            ...
            if(!$model->usernameExists($username) OR !$model->emailExists($email) ){
                ...
                if ($validation->isValide($error)){
                    ...
                    $this->show('default/register' , array('error' => $error , 'departements' =>$departements));
                }
            }
        }
    }
